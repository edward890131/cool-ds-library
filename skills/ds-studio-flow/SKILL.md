...
**做法**：設計師按 topbar 右上「**儲存並更新 · v<N>**」→ 先跳防呆彈窗列出這次改了哪些（v<N>→v<N+1>）、確認才 `POST /api/project/<name>/export` → 寫檔到 `projects/<name>/`（含 `library.version.json` 版本 +1，`source: website`）。**只動這個專案元件庫、不碰共用引擎**：
...
| `library.version.json` | 專案元件庫版本＋changelog history |
...
# 吃回 Figma 改動、套進 projects/<name>/ 後，記一筆專案版本（網站會提示「版本已更新」）：
node sync/bump-version.mjs <name> --source=figma --change="..."
```

**專案元件庫版本化（Phase F）**：網站「儲存並更新」與 sync 回灌都會讓 `projects/<name>/library.version.json` 版本前進、寫 changelog。這是**專案層**版本，共用引擎（`library/`）維持人工升級——升級到共用引擎仍走「結構變動 → upstream-候選 → 人工審核」，不由專案版本自動推。
