...
## Phase F — 專案元件庫版本化（2026-07-17）
釐清「兩個母庫」：**共用引擎**（`library/`，人工升級 🔒）vs **專案元件庫**（`projects/<name>/`＋該案 Figma／codebase，設計師透過網站 cockpit 改）。版本化只作用在專案層，不碰共用引擎——這就是防呆：隨手改 token 永遠不會污染母盤。
- **CTA 升級**：`?project=` 模式下 topbar「匯出到專案」→「**儲存並更新 · v<N>**」。點了先跳**防呆彈窗**列出這次 diff（token/logo/元件/軸增減，add/rm/chg 三色標記）＋「v1→v2」，確認才寫檔。
- **版本模型**：`projects/<name>/library.version.json`＝`{version, history:[{v,at,source,changes}]}`。網站存檔記 `source: website`；Figma 回灌用 `sync/bump-version.mjs <name> --source=figma --change=...` 記 `source: figma`。
- **進站版本彈窗**：網站用 localStorage 記住「上次看到的版本」，發現專案版本被推進過（多半是 Figma 同步）→ 進站跳「版本已更新」列出中間 changelog；首訪靜默記住、自己存的不重跳。
- server：`GET /api/project/<name>` 加回傳 version；`POST .../export` 收 `version.historyEntry` → append `library.version.json`。
